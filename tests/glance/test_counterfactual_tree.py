...
from aix360.algorithms.glance.counterfactual_tree.counterfactual_tree import CounterfactualTree  # Adjust the import as needed
...
def test_counterfactual_tree_initialization(fitted_model):
    tree = CounterfactualTree(model=fitted_model)
...
def test_counterfactual_tree_fit(fitted_model):
    ...
    tree = CounterfactualTree(model=fitted_model, split_features=3)
    tree.fit(data.drop(columns='target'), data['target'], data)
